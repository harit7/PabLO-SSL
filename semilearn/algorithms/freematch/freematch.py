...
class FreeMatch(AlgorithmBase):
    ...
    def init(
        self, T, hard_label=True, ema_p=0.999, use_quantile=True, clip_thresh=False
    ):
        self.T = T
    ...
    def set_hooks(self):
        ...
        self.register_hook(
            FreeMatchThresholingHook(
                num_classes=self.num_classes, momentum=self.args.ema_p
            ),
            "MaskingHook",
        )
        super().set_hooks()
        ...
        with self.amp_cm():
            if self.use_cat:
                (
                    ...
                    feats_x_ulb_w,
                    feats_x_ulb_s,
                ) = self.use_cat_func(x_lb, x_ulb_w, x_ulb_s, num_lb)
            else:
            ...
            self.check_if_use_margin_reg(logits_x_ulb_w, logits_x_ulb_s)

            if self.batch_pl_flag:
                # calculate mask
                mask_batch = self.call_hook(
                    "masking", "MaskingHook", logits_x_ulb=logits_x_ulb_w
                )

                # generate unlabeled targets using pseudo label hook
                pseudo_labels_batch = self.call_hook(
                    "gen_ulb_targets",
                    "PseudoLabelingHook",
                ...
                if self.acc_pseudo_labels_flag:
                    pseudo_labels_batch, mask_batch = self.accumulate_pseudo_labels(
                        idx_ulb, mask_batch, pseudo_labels_batch
                    )
            ...
            unsup_loss = self.consistency_loss(
                logits_x_ulb_s, pseudo_labels_batch, "ce", mask=mask_batch
            )

            # calculate entropy loss
            if mask_batch.sum() > 0:
                ent_loss, _ = entropy_loss(
                    mask_batch, logits_x_ulb_s, self.p_model, self.label_hist
                )
            else:
                ent_loss = 0.0
            # ent_loss = 0.0
    ...
    def load_model(self, load_path):
        ...
        self.hooks_dict["MaskingHook"].p_model = checkpoint["p_model"].cuda(
            self.args.gpu
        )
        self.hooks_dict["MaskingHook"].time_p = checkpoint["time_p"].cuda(self.args.gpu)
        self.hooks_dict["MaskingHook"].label_hist = checkpoint["label_hist"].cuda(
            self.args.gpu
        )

...
from semilearn.datasets.cv_datasets.datasetbase import augment_ds, union_ds
from semilearn.datasets.utils import randomly_split_labeled_basic_dataset


class ThresholdScheduler:
    def __init__(
        self, warmup_epochs, init_thres, final_thres, total_steps, steps_per_epoch
    ):
...
class AlgorithmBase:
    ...
    def __init__(self, args, net_builder, tb_log=None, logger=None, **kwargs):
        ...
        if self.args.bayes:
            self.bayes_optimizer, self.bayes_scheduler, self.quantile_sch = (
                self.set_bayes_optimizer()
            )
        else:
            self.bayes_optimizer, self.bayes_scheduler, self.quantile_sch = (
                None,
    ...
    def set_dataset(self):
        ...
        if need_d_cal or need_d_th:
            ...
            ds1 = dataset_dict["train_lb"]
            ds1.Y = torch.Tensor(ds1.targets)
            ...
            if need_d_cal and need_d_th:
                ...
                # # print(ds11.Y)
                dataset_dict["d_cal_d_th"] = ds1
                dataset_dict["d_cal"] = ds1 # was ds11
                dataset_dict["d_th"] = ds1 # was ds12
                # self.print_fn(f"len(d_cal) = {len(ds11)} and len(d_th) = {len(ds12)}")
                self.print_fn(f"len(d_cal) = {len(ds1)} and len(d_th) = {len(ds1)}")
    ...
    def set_data_loader(self):
        ...
        loader_dict["all_train_lb"] = get_data_loader(
            self.args,
            self.dataset_dict["train_lb"],
            self.args.batch_size,
            data_sampler=None,
        ...
        loader_dict["all_train_ulb"] = get_data_loader(
            self.args,
            self.dataset_dict["train_ulb"],
            self.args.batch_size,
            data_sampler=None,
    ...
    def set_bayes_optimizer(self):
        ...
        bayes_optimizer = optim.Adam(
            self.model.classifier.parameters(), lr=self.args.bayes_args.bayes_lr
        )
    ...
    def accumulate_pseudo_labels(self, idx_ulb, mask, pseudo_labels):
        ...
        self.pseudo_labels[idx_ulb_pl] = pseudo_labels[
            mask_bool
        ]  # overwrite the pseudo label for these points.

        self.mask[idx_ulb] = torch.clamp(
            mask + self.mask[idx_ulb], min=0.0, max=1.0
        )  # update the mask accordingly.

        self.print_fn(
            f"{torch.sum(mask).item()},  {torch.sum( self.mask[idx_ulb] ).item()},{torch.sum(self.mask).item()}"
        )
    ...
    def train(self):
        ...
        self.freq_schedule = self.args.freq_schedule

        device = str(next(self.model.parameters()).device)
        self.device = device

        if self.args.algorithm == "freematch":
        ...
        # Default resume flag will be false!
        if not self.resume:
        ...
        # create a fake epoch run to cache something
        cache.variables["should_use_cache"] = False
        train_lb_loader, train_ulb_loader = (
            self.loader_dict["all_train_lb"],
            self.loader_dict["all_train_ulb"],
        )
        for data_lb in train_lb_loader:
            pass
        for data_ulb in train_ulb_loader:
            pass
        print(":" * 8, "faking epoch is done")
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            cache.variables["should_use_cache"] = False
            ## DO NOT USE CACHE FOR THIS DATA LOADER
            for data_lb, data_ulb in zip(
                ...
                if self.freq_schedule:
                    if self.it < 1000:
                        Freq = self.full_pl_freq // 2
                    elif self.it < 2000:
                        Freq = self.full_pl_freq // 2
                    else:
                        Freq = self.full_pl_freq
                ...
                self.print_fn(f"Time per iter : {time_per_iter:.3f}")
    ...
    def post_hoc_calib_pseudo_labeling(self):
        ...
        # create new d_cal, d_th by data augmentation.
        # take base samples from some initial groundtruth labeled data and newly pseudolabeled data.
        # consistency in the pseudolabel over multiple iterations can be thought of as its accuracy/confidence.
        # so it might be better to select those points.

        # x_ulb, self.pseudo_labels, self.mask

        tic = time()
        aug_calib_data = False

        if aug_calib_data:
            idx_ulb = np.arange(0, self.n_u)
            mask = self.mask.ge(0.0).detach().cpu().numpy()
            plb_idx = idx_ulb[mask]
            plb_targets = self.pseudo_labels.cpu().numpy()[plb_idx]
            # sub sample psuedo labeled data to ensure class balance
            yy, cc = np.unique(plb_targets, return_counts=True)
            pp = max(100, min(cc))
            if pp > 0:
                idx_sel = []
                for y in range(self.num_classes):
                    idx_yy = np.random.choice(
                        plb_idx[plb_targets == y], size=pp, replace=True
                    )
                    idx_sel.append(idx_yy)

                idx_sel = np.concatenate(idx_sel)

                plb_data = self.dataset_dict["train_ulb"].data[idx_sel]
                plb_targets = plb_targets[idx_sel]
                ds_2 = union_ds(self.dataset_dict["d_cal_d_th"], plb_data, plb_targets)
            else:
                ds_2 = self.dataset_dict["d_cal_d_th"]

            n_ = len(ds_2.targets)
            n__ = self.args.n_cal + self.args.n_th
            cf = ceil(n__ / (2 * n_))
            if cf > 1:
                ds_2 = augment_ds(ds_2, cf, cf)

            d_cal, d_th = randomly_split_labeled_basic_dataset(
                ds_2, self.num_classes, size_1=self.args.n_cal, class_balance=True
            )

        else:
            d_cal = self.dataset_dict["d_cal"]
            d_th = self.dataset_dict["d_th"]

        self.cur_calibrator.fit(d_cal, ds_val_nc=d_th, epochs=self.it // 25, it=self.it)

        toc = time()

        self.print_fn(f"Total time in calibrator training : {toc - tic}")
        # >>>>>>>>>>>>>>>>>>>>>>>>>>> END CALIBRATION BLOCK  >>>>>>>>>>>>>>>>>>>>>>>>>>>

        # get pseudo labels and mask here.
        # estimate threshold, pseudo label etc.
        inf_conf = {"feature_key": "x_lb", "idx_key": "idx_lb"}
        val_inf_out_th = self.cur_calibrator.predict(d_th, inference_conf=inf_conf)

        lst_classes = np.arange(0, self.num_classes, 1)
        auto_lbl_conf = self.post_hoc_calib_conf.auto_lbl_conf
        val_idcs = np.arange(0, len(d_th.targets), 1)
        ...
        if self.it % 2000 == 0:
            # refresh cache every k=2000 iter.
            cache.variables["should_use_cache"] = False
        else:
            cache.variables["should_use_cache"] = self.args.should_use_cache  # False

        tic = time()
        inf_conf = {"feature_key": "x_ulb_w", "idx_key": "idx_ulb", "batch_size": 2048}
        ...
        tt = torch.tensor([lst_t_val[y_hat[i]] for i in range(self.n_u)]).to(device)
        ...
        if self.algorithm == "freematch":
            self.freematch_update(unlbld_inf_out["probs"])
            self.print_fn(f"p_model : {self.p_model}")
            self.print_fn(f"lbl_hist : {self.label_hist}")
            self.print_fn(f"time_p : {self.time_p}")
    ...
    def full_pseudo_labeling(self):
        ...
        # compute mask
        mask_full = self.call_hook(
            "masking", "MaskingHook", logits_x_ulb=probs_x_ulb_w, softmax_x_ulb=False
        )

        # generate unlabeled targets using pseudo label hook
        pseudo_labels_full = self.call_hook(
            "gen_ulb_targets",
            "PseudoLabelingHook",
        ...
        if self.algorithm == "freematch":
            self.freematch_update(probs_x_ulb_w)
            self.print_fn(f"p_model : {self.p_model}")
            self.print_fn(f"lbl_hist : {self.label_hist}")
            self.print_fn(f"time_p : {self.time_p}")
    ...
    def log_batch_pseudo_labeling_stats(
        self, mask_batch, pseudo_labels_batch, idx_ulb_batch
    ):
        ...
        if not self.tb_log is None:
            self.tb_log.update(
                {"batch_pl_cov": batch_cov, "batch_pl_acc": batch_acc}, self.it
            )
    ...
    def freematch_update(self, probs_x_ulb):
        ...
        hist = torch.bincount(max_idx.reshape(-1), minlength=self.p_model.shape[0]).to(
            self.p_model.dtype
        )
    ...
    def evaluate(self, eval_dest="eval", out_key="logits", return_logits=False):
        ...
        with torch.no_grad():
            for data in eval_loader:
                ...
                loss = (
                    F.cross_entropy(logits, y, reduction="mean", ignore_index=-1) + Lkl
                )
    ...
    def load_model(self, load_path):
        ...
        """
        # Loading pseudo labels and masks!
    ...
    def check_if_use_bam(self, Lkl, logits, num_lb, rep):
        if self.args.bayes:
            ...
            mean_output_u_w, std_output_u_w = self.bayes_predict(
                self.args.bayes_args, self.model.classifier, rep_u_w
            )
            ...
            if (
                self.args.bayes_args.quantile != -1
                and self.epoch > self.args.bayes_args.q_warmup
            ):
                new_threshold = torch.quantile(
                    pred_std, self.args.bayes_args.quantile
                ).item()
                if self.args.bayes_args.q_queue:
                    self.quan_queue.append(new_threshold)
                    if len(self.quan_queue) > 50:
                        self.quan_queue.pop(0)  # maintain last 50 values
                    new_threshold = np.mean(self.quan_queue)
                    self.args.bayes_args.args.std_threshold = new_threshold

    def check_if_use_margin_reg(self, logits_x_ulb_w, logits_x_ulb_s):
        if self.args.mr:
            ...
            matching_samples_mask = (
                pseudo_label == torch.argmax(logits_x_ulb_s, dim=1)
            ) & (mask == 1)
            ...
            self.loss_margin = (
                self.penalty_loss(
                    matching_logits,
                    matching_pseudo_label,
                    self.args.mr_args.margin_hyperparam,
                )
                * self.args.mr_args.p_penalty
            )
        else:
            self.loss_margin = 0
    ...
    def bayes_predict(args, bayeslayer, reps, T=1):
        ...
        with torch.no_grad():
            with autocast():
                outputs = [
                    torch.softmax(bayeslayer(reps)[0] / T, dim=-1)
                    for _ in range(args.bayes_samples)
                ]

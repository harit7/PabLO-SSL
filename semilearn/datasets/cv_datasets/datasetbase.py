...
def timing(f):
    ...
    def wrap(*args, **kw):
        ...
        result = f(*args, **kw)
        te = time()
        print("func:%s took: %2.8f sec" % (f.__name__, te - ts))
        return result
...
class BasicDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if (
            cache.variables["should_use_cache"]
            and cache_key in cache.cached_data
            and cache.variables["enable_caching"]
        ):
            return cache.cached_data[cache_key]

        img, target = self.__sample__(idx)

        if self.transform is None:
            ret = {"x_lb": transforms.ToTensor()(img), "y_lb": target}
        else:
            ...
            if not self.is_ulb:
                ret = {"idx_lb": idx, "x_lb": img_w, "y_lb": target}
            else:
                if self.alg == "fullysupervised" or self.alg == "supervised":
                    ret = {"idx_ulb": idx, "x_ulb": x_ulb}

                elif self.alg == "pseudolabel" or self.alg == "vat":
                    ret = {"idx_ulb": idx, "x_ulb": x_ulb, "x_ulb_w": img_w}

                elif (
                    self.alg == "pimodel"
                    or self.alg == "meanteacher"
                    or self.alg == "mixmatch"
                ):
                ...
                # elif self.alg == 'sequencematch' or self.alg == 'somematch':
                    ...
                    ret = {
                        ...
                        "x_ulb": x_ulb,
                        "x_ulb_w": img_w,
                        "x_ulb_m": self.medium_transform(img),
            ...
            if cache.variables["enable_caching"]:
                cache.cached_data[cache_key] = ret
...
def union_ds(basic_ds, data, targets):
    data_2 = []
    targets_2 = []
    n1 = len(basic_ds.targets)
    n2 = len(targets) 
    for i in range(n1):
        data_2.append(basic_ds.data[i])
        targets_2.append(basic_ds.targets[i])
        
    for i in range(n2):
        data_2.append(data[i])
        targets_2.append(targets[i])
    
    idcs = np.arange(len(targets_2))
    
    np.random.shuffle(idcs)
    #randomly shuffle
    targets_2 = targets_2[idcs] 
    data_2 = np.array(data_2)[idcs]
      
    basic_ds_2 = BasicDataset(
        basic_ds.alg,
        data_2,
        targets_2,
        basic_ds.num_classes,
        basic_ds.transform_weak,
        False,
        basic_ds.transform_strong,
        basic_ds.transform_strong,
        False,
    )
...
def augment_ds(basic_ds, num_weak, num_strong):
    data, targets = basic_ds.data, basic_ds.targets
    
    #for each point create k replicas
    n = len(targets)
    idcs = [] 
    idcs = np.arange(idcs)
    targets_2 = [] 
    ...
    for i in range(n):
        ...
        for j in range(num_weak):    
            img_w = basic_ds.transform(img)
            data_2.append(img_w)
            targets_2.append(targets[i])
        
        for k in range(num_strong):
            img_s = basic_ds.strong_transform(img)
            data_2.append(img_s)
            targets_2.append(targets[i])
    ...
    basic_ds.data = data_2 
    basic_ds.targets = targets_2 

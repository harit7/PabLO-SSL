...
def over_write_args_passed_in_command(args, args_cp):
    # here we know which arguments are passed in the command and we want to overwrite those
    # i.e. don't want to use the values from the config file for these but want to use the one passed from the command line
    args.epoch = args_cp.epoch
    args.num_train_iter = args_cp.num_train_iter
    args.save_dir = args_cp.save_dir
...
def get_save_name(args):
    ...
    sep = "/"
    lst_kv = []
    lst_kv.append(("P", args.prefix))
    lst_kv.append(("A", args.algorithm))
    lst_kv.append(("D", args.dataset))
    lst_kv.append(("S", args.seed))
    lst_kv.append(("E", args.epoch))
    lst_kv.append(("I", args.num_train_iter))
    lst_kv.append(("nl", args.num_labels))
    lst_kv.append(("bsz", args.batch_size))
    lst_kv.append(("accpl", args.accumulate_pseudo_labels))
    lst_kv.append(("cache", args.should_use_cache))

    lst_kv.append(("aug1", args.aug_1))
    lst_kv.append(("aug2", args.aug_2))

    if args.bayes:
        lst_kv.append(("bam", args.bayes))

    if args.mr:
        lst_kv.append(("mr", args.mr))

    if args.full_pl_flag:

        lst_kv.append(("full_pl", args.full_pl_flag))
        lst_kv.append(("freq", args.full_pl_freq))
        lst_kv.append(("freq_sch", args.freq_schedule))

    else:
        lst_kv.append(("batch_pl", args.batch_pl_flag))

    if args.use_post_hoc_calib:
        lst_kv.append(("phoc", args.use_post_hoc_calib))
        lst_kv.append(("phoc_name", args.post_hoc_name))
        lst_kv.append(("n_cal", args.n_cal))
        lst_kv.append(("nth", args.n_th))
        lst_kv.append(("from", args.take_d_cal_th_from))
        lst_kv.append(("phoc_eps", args.post_hoc_eps))

        lst_kv.append(("phoc_use_prev", args.use_prev_model))

        # if(args.post_hoc_name=='pablo'):
        lst_kv.append(("phoc_l2", args.post_hoc_l2))
        lst_kv.append(("phoc_bs", args.post_hoc_bs))
        lst_kv.append(("phoc_E", args.post_hoc_max_epochs))
        lst_kv.append(("phoc_lr", args.post_hoc_lr))
        lst_kv.append(("phoc_wd", args.post_hoc_wd))

    save_name = f"{sep}".join([f"{k}--{v}" for k, v in lst_kv])
...
def get_config():
    ...
    parser.add_argument("--should_use_cache", type=str2bool, default=False)

    # to use a frequency schedule or keep using fixed frquency.
    parser.add_argument("--freq_schedule", type=str2bool, default=False)
    ...
    parser.add_argument("--post_hoc_name", type=str, default="pablo")
    ...
    parser.add_argument("--post_hoc_bs", type=int, default=64)
    parser.add_argument("--post_hoc_eps", type=str, default=0.05)
    parser.add_argument("--post_hoc_l2", type=float, default=100.0)
    # parser.add_argument("--post_hoc_epochs",type=int, default=500)
    parser.add_argument("--post_hoc_lr", type=float, default=0.01)
    parser.add_argument("--post_hoc_wd", type=float, default=0.01)
    ...
    # begin: bayes cla
    parser.add_argument(
        "--bayes", type=str2bool, default=False, help="Whether to use BaM or not"
    )
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=False,
        help="Whether to use margin regularization or not",
    )
    parser.add_argument("--mr_config", type=str, help="path to the MR config file")
    # end: mr
    ...
    # it is stupid to do so many overwrites but seems like this is the only way.
    ...
    over_write_args_passed_in_command(args, args_cp)
    ...
    if args.mr and not args.mr_config:
        raise ValueError("Margin regularization is true but mr_config is None!")
    if args.mr and args.mr_config:
        args.mr_args = OmegaConf.load(args.mr_config)
...
def main(args):
    ...
    cache.init(args.use_post_hoc_calib)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.use_post_hoc_calib:
        post_hoc_calib_conf = OmegaConf.load(
            f"./config/post-hoc/{args.post_hoc_name}_{args.dataset}.yaml"
        )
        post_hoc_calib_conf.training_conf_g.max_epochs = args.post_hoc_max_epochs
        post_hoc_calib_conf.training_conf_g.batch_size = args.post_hoc_bs
        post_hoc_calib_conf.training_conf_g.learning_rate = args.post_hoc_lr

        post_hoc_calib_conf.training_conf_g.weight_decay = args.post_hoc_wd

        post_hoc_calib_conf.training_conf_t = copy.deepcopy(
            post_hoc_calib_conf.training_conf_g
        )

        post_hoc_calib_conf.l2 = args.post_hoc_l2

        post_hoc_calib_conf.auto_lbl_conf.auto_label_err_threshold = args.post_hoc_eps
